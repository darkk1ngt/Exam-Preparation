...
// Middleware: require producer
const requireProducer = (req, res, next) => {
    if (!req.session.user || req.session.user.role !== 'producer') {
        return res.status(403).json({ error: 'Producer access required' });
    }
    if (!req.session.user.email_verified) {
        return res.status(403).json({ error: 'Email not verified' });
    }
    if (req.session.user.producer_status !== 'approved') {
        return res.status(403).json({ error: 'Producer not approved' });
    }
    next();
};
